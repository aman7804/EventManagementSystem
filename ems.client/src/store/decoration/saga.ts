...
import service from "services/decoration.service";
...
function* listSaga(action: any) {
  try {
    const response: IApiSuccessResponse<IDecorationPagination> = yield call(
...
function* saveSaga(action: any) {
  try {
    const response: IApiSuccessResponse<IDecoration> = yield call(
